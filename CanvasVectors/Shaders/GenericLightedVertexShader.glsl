...
void main()
{
    ...
    gl_Position = projection_matrix * view_matrix * vec4(vertex_position_modelspace, 1);
...
void main()
{
    ...
    // Position of the vertex, in worldspace : M * position
    vec3 Position_worldspace = (vec4(vertex_position_modelspace,1)).xyz;

    // Vector that goes from the vertex to the camera, in camera space.
    // In camera space, the camera is at the origin (0,0,0).

    vec3 vertexPosition_cameraspace = ( view_matrix * vec4(vertex_position_modelspace,1)).xyz;
    ...
    // Normal of the the vertex, in camera space
    normal_cameraspace = ( view_matrix * vec4(vertex_normal_modelspace,0)).xyz; // Only correct if ModelMatrix does not scale the model ! Use its inverse transpose if not.
}